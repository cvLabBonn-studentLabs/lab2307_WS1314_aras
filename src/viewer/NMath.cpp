...
namespace NMath {
  ...
  void orderedRandom(CVector<double>& aResult, const gsl_rng * r) {
    double tmp = 1.0;
    for(int i=aResult.size(); i>0; i--) {
      tmp *= pow( random(r), 1.0/(double)i );
      aResult[i-1] = tmp;
    }
  }

  void stratifiedSampling(CVector<double>& aResult, const gsl_rng * r) {
    double d = 1.0/(double)aResult.size();
    for(int i=0; i<aResult.size(); i++) {
      aResult[i] = (double)i*d+d*random2(r);
    }
  }
  ...
  void randomMultiGauss(const gsl_rng * r, const CMatrix<double>* triA, CVector<double>& res) {
    for(int i = 0; i < res.size(); ++i)
      res(i) = gsl_ran_gaussian(r, 1);
    
    for(int i = res.size()-1; i >= 0; --i) {
      res(i) *= (*triA)(i,i);
      for(int j = 0; j<i; ++j)
	res(i) += res(j)*(*triA)(i,j);
    }
  ...
  ********************************************/
  ...
  void euclideanDistanceTransform(CMatrix<float>& aMatrix, CMatrix<bool>* aExclude) {
    ...
    CVector<float> f(max(aXSize,aYSize));
