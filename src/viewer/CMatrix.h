...
//  void upsampleBicubic(int aNewXSize, int aNewYSize);
  ...
  void cut(CMatrix<T>& aResult,const int x1, const int y1, const int x2, const int y2) const;
  // Copies a rectangular part from the matrix into aResult at position posX,posY
  void cut(CMatrix<T>& aResult, int posX, int posY, const int x1, const int y1, const int x2, const int y2) const;
...
// cut
template <class T>
void CMatrix<T>::cut(CMatrix<T>& aResult,const int x1, const int y1, const int x2, const int y2) const {
...
template <class T>
void CMatrix<T>::cut(CMatrix<T>& aResult, int posX, int posY, const int x1, const int y1, const int x2, const int y2) const {
  for (int y = y1; y <= y2; y++)
    for (int x = x1; x <= x2; x++)
...
void CMatrix<T>::readFromMathematicaFile(const char* aFilename) {
  ...
  // Analyse file ---------------
  aPos = 0;
  if (aData[aPos] != '{') throw EInvalidFileFormatMathematica();
  aPos++;
  for (int y = 0; y < mYSize; y++) {
    if (aData[aPos] != '{') throw EInvalidFileFormatMathematica();
    aPos++;
...
void CMatrix<T>::readFromPNG(const char* aFilename) {
  ...
  if(color_type != PNG_COLOR_TYPE_GRAY) {
    fprintf(stderr, "PNG is not gray\n");
    return;
  }
...
// writeToPNG
template <class T>
void CMatrix<T>::writeToPNG(const char *aFilename) {
  FILE *f;
  ...
  png_bytep row = new png_byte[mXSize];
  
  for(int y = 0; y < mYSize; y++) {
    for(int x = 0; x < mXSize; x++)
      row[x] = (unsigned char)mData[y * mXSize + x];
...
// readFromPNG
template <class T>
void CMatrix<T>::readFromPNGflip(const char* aFilename) {
  png_byte header[8];
  FILE *f = fopen(aFilename, "rb");
  if(!f)
    fprintf(stderr, "error: file %s not found\n", aFilename);
...
// writeToPNG
template <class T>
void CMatrix<T>::writeToPNGflip(const char *aFilename) {
  FILE *f;
  f = fopen(aFilename,"wb");
...
void CMatrix<T>::readFromPDM(const char* aFilename) {
  ...
  aStream >> mYSize;
  aStream >> s;
  if (s != "F") throw EInvalidFileFormat("Float PDM");
  // Adjust size of data structure
  delete[] mData;
  mData = new T[mXSize*mYSize];
  // Read data
...
void CMatrix<T>::readFromIntegerPDM(const char* aFilename) {
  ...
  if (std::string(buffer) != "P9") throw EInvalidFileFormat("PDM");
  // Remove comments
  while (getc(aStream) == '#') {
    while (getc(aStream) != '\n');
  }
  ...
  fscanf(aStream,"%s",buffer);
  if (std::string(buffer) != "I") throw EInvalidFileFormat("Integer-PDM");
  // Adjust size of data structure
  delete[] mData;
  mData = new T[mXSize*mYSize];
  getc(aStream);
...
// readFromTXT
template <class T>
void CMatrix<T>::readFromTXT(const char* aFilename) {
  std::ifstream aStream(aFilename);
  // read header
...
//writeToTXT
template <class T>
void CMatrix<T>::writeToTXT(const char* aFilename) {
  std::ofstream aStream(aFilename);
  // write header
  aStream << mXSize << " " << mYSize << std::endl;
  // write data
...
CVector<T> operator*(const CVector<T>& aVector, const CMatrix<T>& aMatrix) {
  if (aMatrix.ySize() != aVector.size())
    throw EIncompatibleMatrices(aVector.size(),1,aMatrix.xSize(),aMatrix.ySize());
  CVector<T> result(aMatrix.xSize(),0);
  for (int x = 0; x < aMatrix.xSize(); x++)
    for (int y = 0; y < aMatrix.ySize(); y++)
