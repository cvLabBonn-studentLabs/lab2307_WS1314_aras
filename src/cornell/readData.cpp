...
static bool READ_FROM_PNG = true;
...
private:
    ...
    // return true if data retrieving was successful
    bool readNextPNG(cv::Mat& rgb_img, cv::Mat& depth_img) {
        stringstream ss;
        ss << currentFrameNum;
        ...
        rgb_img = cv::imread((char*)fileName_RGBD.c_str(),1);
        //cv::Mat colorArr[3];
        //cv::split(rgbImage, colorArr);
        
        depth_img = cv::imread((char*)fileName_Depth.c_str(),-1);
        
        if (rgb_img.data == NULL) {
            printf("ERROR! Unable to open file %s.\n", (char*)fileName_RGBD.c_str());
            exit(1);
        }
        if (depth_img.data == NULL) {
            printf("ERROR! Unable to open file %s.\n", (char*)fileName_Depth.c_str());
            exit(1);
        }
        /*
...
public:
    ...
    static int xPixelFromCoords(double x, double y, double z)
    {
    ...
    static int yPixelFromCoords(double x, double y, double z)
    {
      return (int) (125.5357201011431 + 0.0002153447766 * x - 0.1184874093530 * y - 0.0022134485957 * z);
    }

    // return true if data retrieving was successful
    bool readNextFrame(double **data, double **pos_data, int **data_CONF, int *data_pos_CONF, cv::Mat& rgb_img, cv::Mat& depth_img) {
        ...
        bool status_RGBD = readNextPNG(rgb_img, depth_img);
        //}
        if (status_RGBD) {
